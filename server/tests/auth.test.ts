...
const decodeToken = (token: string) =>
  jwt.verify(token, config.jwt.secret) as jwt.JwtPayload & { userId?: string; role?: string };
...
await runTest('Test 1: Successful registration creates user with hashed password and returns JWT', async () => {
  ...
  assert.strictEqual((result.user as any).passwordHash, undefined, 'passwordHash must NEVER be returned');
  assert.strictEqual((result.user as any).password, undefined, 'password must NEVER be returned');
...
await runTest('Test 5: Successful login with correct credentials returns safe user and valid JWT', async () => {
  ...
  assert.strictEqual(result.user.email, TEST_EMAIL.toLowerCase());
  assert.strictEqual((result.user as any).passwordHash, undefined, 'passwordHash must NEVER be returned on login');
...
await runTest('Test 12: Controller endpoints (register, login, me) never leak passwordHash', async () => {
  ...
  const meReq: any = { user: regRes.body.data.user };
