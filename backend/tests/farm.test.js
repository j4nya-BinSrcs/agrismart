...
await runTest('Test 2: POST /farms without JWT returns 401 Unauthorized', async () => {
  const req = { headers: {}, body: { name: 'Test Farm' } };
  const res = createMockRes();
  let caughtError = null;
...
await runTest('Test 4: User A gets own farms list (200)', async () => {
  ...
  assert(res.body.data.farms.some((f) => f.id === farmA.id));
});

await runTest('Test 5: User A gets farm by ID (200)', async () => {
  const req = { user: userA, params: { farmId: farmA.id } };
  const res = createMockRes();
...
await runTest('Test 10: User A lists fields under own farm (200)', async () => {
  ...
  assert(res.body.data.fields.some((f) => f.id === fieldA.id));
});

await runTest('Test 11: User A gets field by ID (200)', async () => {
  const req = { user: userA, params: { farmId: farmA.id, fieldId: fieldA.id } };
  const res = createMockRes();
...
await runTest('Test 16: User A lists zones under field (200)', async () => {
  ...
  assert(res.body.data.zones.some((z) => z.id === zoneA.id));
});

await runTest('Test 17: User A gets zone by ID (200)', async () => {
  const req = { user: userA, params: { farmId: farmA.id, fieldId: fieldA.id, zoneId: zoneA.id } };
  const res = createMockRes();
...
await runTest('Test 20: GET /farms/:farmId/overview returns complete farm hierarchy with grouped fields and zones', async () => {
  ...
  assert(res.body.data.farm.fields[0].zones.some((z) => z.id === zoneA.id));
});
...
await runTest('Test 26: Deleting a Field cascades deletion to all of its child Zones', async () => {
  ...
  const tempZone1 = await zoneService.createZone(userA.id, farmA.id, tempField.id, {
    name: 'Temp Zone 1',
    areaAcres: 4.0,
  });

  const tempZone2 = await zoneService.createZone(userA.id, farmA.id, tempField.id, {
    name: 'Temp Zone 2',
    areaAcres: 4.0,
  });
