...
const field2 = await fieldService.createField(userA.id, farmA.id, {
  name: 'Field 2 (Wheat)',
  areaAcres: 10.0,
  crop: 'Wheat',
  soilType: 'loam',
...
const zone1 = await zoneService.createZone(userA.id, farmA.id, field1.id, {
  name: 'Zone 1A',
  areaAcres: 5.0,
  irrigationMethod: 'Drip',
  soilMoisture: 28,
...
const zone2 = await zoneService.createZone(userA.id, farmA.id, field1.id, {
  name: 'Zone 1B',
  areaAcres: 5.0,
  irrigationMethod: 'Drip',
  soilMoisture: 32,
...
await runTest('Test 12: Zero fabricated metrics (no fake CO2 kg, no fake chemical reduction kg)', async () => {
  ...
  assert.strictEqual(summary.metrics.carbonOffsetKg, undefined);
  assert.strictEqual(summary.metrics.chemicalReductionKg, undefined);
  assert.strictEqual(summary.metrics.overallScore, undefined);
