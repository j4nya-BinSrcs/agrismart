 ...
 */
export const parseRootDepthMm = (input) => {
 ...
 */
export const resolveCropCoefficient = (cropName = '', growthStage = '') => {
 ...
 */
export const calculateET0 = (maxTemp, minTemp, currentTemp, latitude = 22.56) => {
 ...
 */
export const calculateEffectiveRainfallHeuristic = (forecastRainMm, rainProbability) => {
 ...
 */
export const evaluateZone = (zoneInput, weatherData, et0) => {
  const {
  ...
  const efficiency = IRRIGATION_EFFICIENCIES[methodKey] || IRRIGATION_EFFICIENCIES.drip;
...
export const irrigationService = {
  ...
  async generatePlan(payload = {}) {
    ...
    const weatherData = payload.weatherData || (await weatherService.getForecast(latitude, longitude));
      ...
      zonesToProcess = [
        {
          ...
          name: `${crop} Field`,
          crop,
          growthStage: growthStage || 'Active Growth',
    ...
    return {
      ...
      weatherContext: {
        ...
        location: weatherData.location
          ? `${weatherData.location.latitude.toFixed(2)}, ${weatherData.location.longitude.toFixed(2)} (${weatherData.location.timezone || 'Asia/Kolkata'})`
