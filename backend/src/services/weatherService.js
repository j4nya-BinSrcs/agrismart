 ...
 */
export const mapWmoCodeToCondition = (code) => {
 ...
 */
export const validateCoordinates = (latInput, lonInput) => {
 ...
 */
const formatHourlyData = (hourly, startIndex, count = 24) => {
...
export const weatherService = {
   ...
   */
  async getForecast(latParam, lonParam) {
    ...
    try {
      ...
      logger.error(`Open-Meteo network connectivity failed: ${err.message}`);
      throw ApiError.internal(`Failed to connect to weather data provider: ${err.message}`);
    } finally {
      clearTimeout(timeoutId);
    }
    ...
    try {
      ...
      logger.error(`Failed to parse Open-Meteo response JSON: ${jsonErr.message}`);
    ...
    const currentRainProb = daily?.precipitation_probability_max?.[0] ?? (current.rain > 0 ? 80 : 10);
